...
const ProfileCreationStep3: React.FC<ProfileCreationStep3Props> = ({ onNext }) => {
  return (
    <div className="min-h-screen bg-[#1C232E] flex flex-col items-center justify-center w-full relative">
      ...
      <div className="flex flex-col items-center flex-1 w-full max-w-5xl mx-auto">
        ...
        {/* Subtitle */}
        <h2 className="text-2xl font-[Cabin,sans-serif] text-[#EBD5BD] font-normal mb-8 mt-2">Next, a quick check for safety & ID</h2>
        {/* Form */}
        <form className="w-full max-w-2xl flex flex-col gap-5" onSubmit={e => { e.preventDefault(); onNext(); }}>
          <div>
            <label className="block text-[#EBD5BD] text-base mb-2">What is your pet's microchip number?</label>
          ...
          <div>
            <label className="block text-[#EBD5BD] text-base mb-2">Any quirks, meds, or special needs?</label>
            <input type="text" placeholder="Type here" className="w-full rounded-md px-4 py-3 text-base bg-white text-black focus:outline-none" />
          </div>
